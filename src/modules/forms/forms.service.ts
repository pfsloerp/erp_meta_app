...
export class MetaAppFormsService {
  ...
  async getAll(paginatedArg: PaginatedArgType) {
    const { items, total } = await this.formsEntityService.getAllForms(paginatedArg);
    return withResponseCode(HttpStatus.OK).items(items, { total });
  }

  async create(body: { form: { name: string; description?: string; content: Record<string, unknown> }; additionalInfo?: Record<string, unknown> }) {
  ...
  async update(id: string, body: { form: { name?: string; description?: string; content?: Record<string, unknown> }; additionalInfo?: Record<string, unknown> }) {
    const payload: Record<string, unknown> = {};
    if (body.form.name !== undefined) payload.name = body.form.name;
    if (body.form.description !== undefined) payload.description = body.form.description;
    if (body.form.content !== undefined) payload.content = body.form.content;
    if (body.additionalInfo !== undefined) payload.additionalInfo = body.additionalInfo;

    const result = await this.formsEntityService.updateForm(id, payload, { throw: true });
    return withResponseCode(HttpStatus.OK).item(result);
  }

  async delete(id: string) {
    await this.formsEntityService.deleteForm(id, { throw: true });
